...
N_COMPANIES   = 50
SEED          = 42
REQUEST_DELAY = 0.6   # EDGAR rate-limits if too fast
...
def fetch_item1a(ticker: str, year: int, cik=None):
    """
    grabs Item 1A from the 10-K for fiscal year `year`.
    most companies file FY-Y in early Y+1, so search window is Jul Y to Jun Y+1.
    returns (text, filing_date) or (None, None) on failure.
    """
    try:
...
def compute_volatility(ticker: str, filing_date) -> float | None:
    """annualized realized vol = std(log daily returns) * sqrt(252), 252 days post-filing"""
    try:
...
def fetch_financial_ratios(ticker: str, year: int) -> dict:
    """debt/equity, roa, current ratio, log market cap. lots of NaNs, imputed downstream"""
...
def run_pipeline():
    ...
    for ticker, year, cik in tqdm(pairs, desc="Fetching filings"):
        ...
        # financial ratios are nice to have but NaNs are fine, imputed later
    ...
    if df_targets.empty:
        print("No observations collected. Check EDGAR connectivity.")
        return
    ...
    print(f"\nDone. {len(df_filings)} obs ({skipped} skipped).")
    print(f"  train={train_mask.sum()}  val={val_mask.sum()}  test={test_mask.sum()}")
    print(f"  vol threshold (train 75th): {threshold:.4f}")
    print(f"  high-vol rate: {df_targets['high_volatility'].mean():.1%}")
